...
#查询结果中答案的存在与否
	...
	def _answer_cover(self):
		...
		base_url = self._words_url + first_letter + '/'

		try:
			page = pq(base_url)
			maxpage_href = pq(page('div.content p.pagination a')[-1]).attr('href')
			maxpage = int(get_number.search(maxpage_href).group())/100
		except Exception, e:
			return None
